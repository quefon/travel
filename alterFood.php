...
<head>
	...
	<script>
		...
		function execute(){
			if ( confirm('確定要刪除此資料？') ){
				location.href="alterFoodAction.php?ID="+ID+"&how=2";
			}
			else
...
<body>
	...
	<div id="body">
	<div class="form">
		<form name="food" method="GET" action="alterFood.php">
	...
	<?php
        ...
        if ( $rownum==0 ){
            ...
            $datalist=$dbgo->query($sql);
            echo "<table border='1'><tr><th>編號</th><th>美食名稱</th><th>動作</th></tr>";
            while ( $row=$datalist->fetch() ) {
                $ID=$row['ID'];
                ?><script>ID='<?php echo $ID;?>'</script><?php
                echo "<tr><td>".$row['ID']."</td><td>".$row['名稱']."</td>";
                echo "<td><a href='alterFoodAction.php?ID=".$row['ID']."&how=1'><input class='button' type='button' value='修改'></a>
