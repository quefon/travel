...
<head>
	...
	<script>
		...
		function execute(){
			if ( confirm('確定要刪除此資料？') ){
				location.href="alterFunAction.php?ID="+ID+"&how=2";
			}
			else
...
<body>
	...
	<div id="body">
	<div class="form">
		<form name="fun" method="GET" action="alterFun.php">
	...
	<?php
        ...
        if ( $rownum==0 ){
            ...
            echo "<table border='1'><tr><th>編號</th><th>景點名稱</th><th>動作</th></tr>";
            while ( $row=$datalist->fetch() ) {
                $ID=$row['ID'];
                echo "<tr><td>".$row['ID']."</td><td>".$row['名稱']."</td>";
                echo "<td><a href='alterFunAction.php?ID=".$row['ID']."&how=1'><input class='button' type='button' value='修改'></a>
