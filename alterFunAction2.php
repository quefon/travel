...
<body>
	...
	<?php
        ...
        $sql="update 樂 SET 名稱='$name' WHERE ID='$ID'";
        $dbh->exec($sql);
        $sql="update 樂 SET 分類='$category'  WHERE ID='$ID'";
        $dbh->exec($sql);
        $sql="update 樂 SET 地址_編號='$num'  WHERE ID='$ID'";
        $dbh->exec($sql);
        $sql="update 樂 SET 地址_其他='$adress'  WHERE ID='$ID'";
        $dbh->exec($sql);
        $sql="update 樂 SET 電話='$phone'  WHERE ID='$ID'";
        $dbh->exec($sql);
        $sql="update 樂 SET 網站='$web'  WHERE ID='$ID'";
        $dbh->exec($sql);
        $sql="update 樂 SET 位置_經度='$longitude'  WHERE ID='$ID'";
        $dbh->exec($sql);
        $sql="update 樂 SET 位置_緯度='$latitude'  WHERE ID='$ID'";
        $dbh->exec($sql);
        ...
        foreach ( $dbh->query($sqlA) as $valueA ) {
            ...
            $sql_insert="update 距離食樂 SET 距離='$distance'  WHERE 食ID='$RID' AND 樂ID='$ID'";
            $dbh->exec($sql_insert);
        }
        $sqlB="SELECT * FROM 住";
        foreach ( $dbh->query($sqlB) as $valueB ) {
            ...
            $distance=getDistance($longitude,$latitude,$lng2,$lat2);
            $distance = number_format($distance,5);
            $sql_insert="update 距離樂住 SET 距離='$distance'  WHERE 住ID='$HID' AND 樂ID='$ID'";
            $dbh->exec($sql_insert);
        }
        if ( $dbh->errorCode()!=00000 ) {
            die("<script> alert(\"修改失敗\"); location.href=\"alterFun.php\";</script>");
        }
        else{
