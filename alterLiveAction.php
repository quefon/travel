...
<body>
	...
	<?php
		...
		if ( $how==1 ){
			echo "<div class='one'><span>修改住資料</span></div>";
			$sql="SELECT * FROM 住 WHERE ID='$ID'";
			foreach ( $dbh->query($sql) as $value ) {
				echo "<div class='form'><form name='live' action='alterLiveAction2.php?ID=$ID&how=1&' method='GET'>"
			...
			echo "<div class='one'><span>刪除住資料</span></div>";
			$sql="DELETE FROM 住 WHERE ID='$ID'";
			$dbh->exec($sql);
			if ( $dbh->errorCode()!=00000 ){
				die("<script> alert(\"刪除失敗\"); location.href=\"alterLive.php\";</script>");   
