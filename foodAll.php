...
<body>
	...
	<?php
        ...
        $num="SELECT * FROM 食,鄉鎮市區 WHERE (ID='$ID') AND (鄉鎮市區.編號=食.地址_編號)";
        ...
        foreach ( $dbh->query($sql) as $value ) {
            echo "<tr><th><span>".$value['名稱']."</span></th></tr>"
                ...
                ."<tr><td><b>營業時間：</b>".$value['營業時間']."</td></tr>"
                ."<tr><td><b>電話：</b>".$value['電話']."</td></tr>";
            echo "</table>";
            echo "<div class='picture'><br><img src='images/foodAll/".$value['ID'].".jpg'></div><br></div>";
        }
        echo "<div class='link'>";
        echo "<ul>";
        ...
        $sql="SELECT * FROM 食 WHERE ID='$ID'";//map
        foreach ( $dbh->query($sql) as $value ) {
        ...
        $sql="SELECT * FROM 食,天氣 WHERE (ID='$ID') AND (天氣.編號=食.地址_編號)";
        foreach ( $dbh->query($sql) as $value ) {
        ...
        $sql="SELECT * FROM 食,鄉鎮市區 WHERE (食.ID='$ID') AND (鄉鎮市區.編號=食.地址_編號)";
        $row=$dbh->query($sql)->fetch();
        echo "<b>".$row['鄉鎮市區']."其他住宿</b><br>";
        $sql="SELECT * FROM 食,住 WHERE (食.ID='$ID') AND (食.地址_編號=住.地址_編號)";
        ...
        echo "<b>小於5km</b><br>";
        $sql="SELECT COUNT(*) FROM 距離食住,住 WHERE (距離食住.食ID='$ID') AND (距離食住.距離<5) AND (距離食住.住ID=住.ID)";
        $num=$dbh->query($sql)->fetchcolumn();
        //echo $num;
        if ( $num==0 )
            echo "無此資料";
        else{
            $sql="SELECT * FROM 距離食住,住 WHERE (距離食住.食ID='$ID') AND (距離食住.距離<5) AND (距離食住.住ID=住.ID)";
            $datalist=$dbh->query($sql);
            while ( $row=$datalist->fetch() ) {
                echo "<a href='liveAll.php?ID=".$row['住ID']."'>".$row['名稱']."</a><br>";}
        }
        echo "</div>";
        echo "<div id='D10A' class='distance' style='display:none'>";
        echo "<b>5到10km</b><br>";
        $sql="SELECT COUNT(*) FROM 距離食住,住 WHERE (距離食住.食ID='$ID') AND (距離食住.距離>=5) AND (距離食住.距離<10) AND (距離食住.住ID=住.ID)";
        $num=$dbh->query($sql)->fetchcolumn();
        //echo $num;
        if ( $num==0 )
            echo "無此資料";
        else{
            $sql="SELECT * FROM 距離食住,住 WHERE (距離食住.食ID='$ID') AND (距離食住.距離>=5) AND (距離食住.距離<10) AND (距離食住.住ID=住.ID)";
            $datalist=$dbh->query($sql);
            while ( $row=$datalist->fetch() ) {
                echo "<a href='liveAll.php?ID=".$row['住ID']."'>".$row['名稱']."</a><br>";}
        }
        ...
        $sql="SELECT * FROM 食,樂 WHERE (食.ID='$ID') AND (食.地址_編號=樂.地址_編號)";
        $datalist=$dbh->query($sql);
        while ( $row=$datalist->fetch() ) {
        ...
        if ( $num==0 )
            ...
            $sql="SELECT * FROM 距離食樂,樂 WHERE (距離食樂.食ID='$ID') AND (距離食樂.距離<5) AND (距離食樂.樂ID=樂.ID)";
            $datalist=$dbh->query($sql);
            while ( $row=$datalist->fetch() ) {
                echo "<a href='funnyAll.php?ID=".$row['樂ID']."'>".$row['名稱']."</a><br>";}
        }
        echo "</div>";
        echo "<div id='D10B' class='distance' style='display:none'>";
        echo "<b>5到10km</b><br>";
        $sql="SELECT COUNT(*) FROM 距離食樂,樂 WHERE (距離食樂.食ID='$ID') AND (距離食樂.距離>=5) AND (距離食樂.距離<10) AND (距離食樂.樂ID=樂.ID)";
        $num=$dbh->query($sql)->fetchcolumn();
        if ( $num==0 )
            echo "無此資料";
        else{
            $sql="SELECT * FROM 距離食樂,樂 WHERE (距離食樂.食ID='$ID') AND (距離食樂.距離>=5) AND (距離食樂.距離<10) AND (距離食樂.樂ID=樂.ID)";
            $datalist=$dbh->query($sql);
            while ( $row=$datalist->fetch() ) {
                echo "<a href='funnyAll.php?ID=".$row['樂ID']."'>".$row['名稱']."</a><br>";}
        }
