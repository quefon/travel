...
<html>
	...
	<?php
        ...
        if ( $township=="" )
            $sql="SELECT count(*) FROM 樂,鄉鎮市區 WHERE 地址_其他 LIKE '...'";
        else if ( $township=="全部" )
            $sql="SELECT count(*) FROM 樂,鄉鎮市區 WHERE (鄉鎮市區.縣市='$city') AND (鄉鎮市區.編號=樂.地址_編號)";
        else
            $sql="SELECT count(*) FROM 樂,鄉鎮市區 WHERE (鄉鎮市區.鄉鎮市區='$township') AND (鄉鎮市區.編號=樂.地址_編號)";

        $rs=$dbh->query($sql);
        $pagesize=10;  //每頁顯示筆數
        $rownum=$rs->mysqli_stmt_num_rows (); //獲取總數
        ...
        if ( $rownum==0 ){
            ...
            if ( $township=="全部" )
                $sql="SELECT * FROM 樂,鄉鎮市區 WHERE (鄉鎮市區.縣市='$city') AND (鄉鎮市區.編號=樂.地址_編號) Limit ".($page-1)*$pagesize.",".$pagesize;
            else
                $sql="SELECT * FROM 樂,鄉鎮市區 WHERE (鄉鎮市區.鄉鎮市區='$township') AND (鄉鎮市區.編號=樂.地址_編號) Limit ".($page-1)*$pagesize.",".$pagesize;

            $datalist=$dbh->query($sql);
            echo "<table><tr><th>景點名稱</th></tr><tr>";
            while ( $row=$datalist->fetch() ) {
                echo "<td><a href='funnyAll.php?ID=".$row['ID']."'>".$row['名稱']."</a></td></tr>";
            }
            echo "</table>";
