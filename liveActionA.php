...
<html>
	...
	<?php
        ...
        if($dbh->connect_errno){
			...
			if ( $township=="" )
				$sql="SELECT count(*) FROM 住,鄉鎮市區 WHERE 地址_其他 LIKE '...'";
			else if ( $township=="全部" )
				$sql="SELECT count(*) FROM 住,鄉鎮市區 WHERE (鄉鎮市區.縣市='$city') AND (鄉鎮市區.編號=住.地址_編號)";
			else
			...
			if ( $rownum==0 ){
				...
				if ( $township=="全部" )
					$sql="SELECT * FROM 住,鄉鎮市區 WHERE (鄉鎮市區.縣市='$city') AND (鄉鎮市區.編號=住.地址_編號) Limit ".($page-1)*$pagesize.",".$pagesize;
				else
					$sql="SELECT * FROM 住,鄉鎮市區 WHERE (鄉鎮市區.鄉鎮市區='$township') AND (鄉鎮市區.編號=住.地址_編號) Limit ".($page-1)*$pagesize.",".$pagesize;			
			
				$datalist=$dbh->query($sql);
				echo "<table><tr><th>住宿名稱</th></tr><tr>";
				while ( $row=$datalist->fetch() ) {
					echo "<td><a href='liveAll.php?ID=".$row['ID']."'>".$row['名稱']."</a></td></tr>";
				}
				echo "</table>";
