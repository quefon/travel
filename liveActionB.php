	...
	<?php
        ...
        if($dbh->connect_errno){
			...
			if ( !isset($_GET['type']) ) 
				...
				$sql="SELECT count(*) FROM 住 WHERE 分類 IN ('".implode("','", $_GET['type'])."')";
			...
			if ( $rownum==0 ){
				...
				$sql="SELECT * FROM 住 WHERE 分類 IN ('".implode("','", $_GET['type'])."') Limit ".($page-1)*$pagesize.",".$pagesize;			
				$datalist=$dbh->query($sql);
				echo "<table><tr><th>住宿名稱</th></tr><tr>";
				while ( $row=$datalist->fetch() ) {
					echo "<td>".$row['分類']."  <a href='liveAll.php?ID=".$row['ID']."'>".$row['名稱']."</a></td></tr>";
				}
				echo "</table>";
